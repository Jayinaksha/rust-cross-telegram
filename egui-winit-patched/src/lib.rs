...
impl State {
    ...
    pub fn on_window_event(
        ...
        match event {
            ...
            WindowEvent::Ime(ime) => {
                // On Android, ALL text input is handled via GameActivity's
                // TextInputState sync in the app wrapper.  Suppress every IME
                // composition event here to avoid double input.
    ...
    fn on_keyboard_input(&mut self, event: &winit::event::KeyEvent) {
        ...
        if let Some(text) = &text {
            ...
            if !text.is_empty() && text.chars().all(is_printable_char) {
                ...
                // On Android, text input is handled via GameActivity's TextInputState
                // sync in the app wrapper. Suppress Event::Text here to avoid double input.
                #[cfg(not(target_os = "android"))]
                {
